# Feeds the stylix base16 palette into caelestia as a "custom" scheme.
#
# Caelestia ships its schemes as data files inside the CLI package rather than
# reading them from the config, so the only way to add one is to graft it into
# the source tree before the package builds.
{
  pkgs,
  inputs,
  config,
  ...
  scheme = pkgs.writeText "custom-dark.txt" ''
  ...
  upstream = inputs.caelestia-cli.packages.${pkgs.stdenv.hostPlatform.system}.default;
in {
  programs.caelestia.cli.package = upstream.overrideAttrs (old: {
    # The palette is part of the derivation's identity, so a theme edit has to
    # produce a different store path or the old scheme stays cached.
    name = "${old.name or "caelestia-cli"}-stylix-${builtins.substring 0 8 (builtins.hashString "sha256" (builtins.toJSON colors))}";
    postUnpack = ''
      mkdir -p $sourceRoot/src/caelestia/data/schemes/custom/main
      cp ${scheme} $sourceRoot/src/caelestia/data/schemes/custom/main/dark.txt
    '';
  });
}
