...
- **A static site is built by a standard generator or framework, never a hand-rolled script.** A bespoke builder re-implements markdown, front matter and URL rewriting badly, and every bug in it is owned forever, where Astro or an equivalent arrives with all three solved and other people maintaining them. This scopes the entry above: avoiding a build step is worth less than avoiding a build step nobody else understands.
