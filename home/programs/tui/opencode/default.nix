{
  ...
  # Single source of truth is nixos/ollama.nix: a provider entry for a model the
  # host never pulls is a menu option that fails at request time.
  ...
  ollamaProvider = lib.optionalAttrs (ollamaModels != []) {
    ollama = {
      ...
      name = "Ollama (local)";
      options = {
        baseURL = "http://127.0.0.1:${toString osConfig.services.ollama.port}/v1";
      };
      models = lib.genAttrs ollamaModels (tag: {
        name = ollamaModelNames.${tag} or tag;
      });
    };
  };
in {
